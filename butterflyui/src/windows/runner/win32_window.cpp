  ...
  switch (message) {
    ...
    case WM_GETMINMAXINFO: {
      if (custom_frame_) {
        ...
        if (monitor != nullptr && minmax != nullptr) {
          MONITORINFO info{};
          info.cbSize = sizeof(MONITORINFO);
          if (GetMonitorInfo(monitor, &info)) {
            const RECT& monitor_rect = info.rcMonitor;
            const RECT& work_rect = info.rcWork;
...
void Win32Window::SetCustomFrame(bool enabled) {
...
bool Win32Window::IsCustomFrame() const {
  return custom_frame_;
}
...
bool Win32Window::IsMaximized() const {
  ...
  return IsZoomed(window_handle_);
}
