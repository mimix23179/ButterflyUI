...
Widget buildColumnControl(
  ...
  final spacing =
      ...
      tokens.number('layout', 'column_spacing') ??
      tokens.number('spacing', 'md') ??
      6.0;
  ...
  final columnChildren = _buildFlexChildren(
    sourceChildren,
    Axis.vertical,
    spacing,
    mainAxisSize,
  ...
  if (clipBehavior != null) {
    column = ClipRect(clipBehavior: clipBehavior, child: column);
  }
  return column;
}
...
CrossAxisAlignment _parseCrossAxisAlignment(
  ...
  switch (s) {
    ...
    case 'baseline':
      // Baseline is unsupported for vertical flex and would assert at runtime.
      return CrossAxisAlignment.start;
  }
  return fallback;
}
