...
class _EditorWorkspaceControlState extends State<_EditorWorkspaceControl> {
  ...
  void initState() {
    super.initState();
    _docs = <_EditorDoc>[];
    final rawDocs = widget.props['documents'] ?? widget.props['tabs'] ?? const [];
    if (rawDocs is List) {
      for (var i = 0; i < rawDocs.length; i += 1) {
        _docs.add(_parseDoc(rawDocs[i], i));
      }
    }
    _activeDocId = (widget.props['active_id'] ?? (_docs.isNotEmpty ? _docs.first.id : '')).toString();
  }
