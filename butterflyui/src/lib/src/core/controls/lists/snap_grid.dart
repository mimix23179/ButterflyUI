...
Widget buildSnapGridControl(
  ...
  return buildRuntimePropsControl(
    ...
    sendEvent: sendEvent,
    stateBuilder: (liveProps) => <String, Object?>{
      ...liveProps,
      'has_child': rawChildren.whereType<Map>().isNotEmpty,
    },
    builder: (liveProps) {
      Widget? child;
      ...
      final spacing = (coerceDouble(liveProps['spacing']) ?? 16).clamp(
        2.0,
        200.0,
      );
      final subdivisions = (coerceOptionalInt(liveProps['subdivisions']) ?? 1)
          .clamp(1, 16);
      final lineColor =
          coerceColor(liveProps['line_color']) ?? const Color(0x22FFFFFF);
      final majorLineColor =
          coerceColor(liveProps['major_line_color']) ?? const Color(0x44FFFFFF);
      final background = coerceColor(liveProps['background']);
      final lineWidth = (coerceDouble(liveProps['line_width']) ?? 1.0).clamp(
        0.25,
        8.0,
      );
      final majorLineWidth =
          (coerceDouble(liveProps['major_line_width']) ?? 1.2).clamp(0.25, 8.0);
      final showGrid = liveProps['show_grid'] == null
          ? true
          : (liveProps['show_grid'] == true);
      final emitOnPress = liveProps['emit_on_press'] == true;
      final emitOnDrag = liveProps['emit_on_drag'] == true;
      final emitOnHover = liveProps['emit_on_hover'] == true;
