...
class StylePack {
  ...
  final ThemeData Function(ThemeData base, StylingTokens tokens)? themeBuilder;
  final Map<String, StyleControlOverride> overrides;
  final Widget Function(BuildContext context, StylingTokens tokens)? background;
  final Widget Function(BuildContext context, StylingTokens tokens, Widget child)?
  wrapRoot;
  ...
  ThemeData applyTheme(StylingTokens tokens) {
    final base = tokens.buildTheme();
    return themeBuilder?.call(base, tokens) ?? base;
  }

  StylingTokens buildTokens(Map<String, Object?> overrides) {
    final merged = StylingTokens.mergeMaps(defaultTokens, overrides);
    return StylingTokens(merged);
  }
}
...
class StylePackRegistry {
  ...
  StylingTokens buildTokens(String? name, Map<String, Object?> overrides) {
