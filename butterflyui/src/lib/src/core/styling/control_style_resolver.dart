...
class ResolvedControlStyle {
  ...
  Map<String, Object?> slot(String slot, {String? state}) {
    ...
    merged = StylingTokens.mergeMaps(
      merged,
    ...
    final stateLayer = ControlStyleResolver._stateMap(states, resolvedState);
    ...
    merged = StylingTokens.mergeMaps(
      merged,
      _readSlotMap(local, normalizedSlot),
    );

    final localStates = _asMap(local['states']);
    final localStateLayer = ControlStyleResolver._stateMap(
      localStates,
      resolvedState,
    );
    if (localStateLayer.isNotEmpty) {
      merged = StylingTokens.mergeMaps(
        merged,
        _readSlotMap(localStateLayer, normalizedSlot),
      );
    }
    return merged;
  }
...
class ControlStyleResolver {
  static ResolvedControlStyle resolve({
    ...
    if (styleSlots.isNotEmpty) {
      ...
      local['slots'] = StylingTokens.mergeMaps(localSlots, styleSlots);
    }
  ...
  static List<String> _resolveClasses(
    ...
    addRaw(local['class_name']);
    addRaw(local['class']);
    addRaw(props['classes']);
    addRaw(props['class_name']);
    addRaw(props['class']);
    return values;
  }
  ...
  static Map<String, Object?> _stateMap(
    Map<String, Object?> states,
    String resolvedState,
  ) {
    final direct = _asMap(states[resolvedState]);
    if (direct.isNotEmpty) return direct;
    if (resolvedState == 'focused') {
      return _asMap(states['focus']);
    }
    if (resolvedState == 'focus') {
      return _asMap(states['focused']);
    }
