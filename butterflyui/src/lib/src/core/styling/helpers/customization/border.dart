...
class _BorderControlState extends State<_BorderControl>
  ...
  void _syncFromProps(
    ...
    _color =
        coerceColor(effective['color'] ?? effective['border_color']) ??
        const Color(0xff64748b);
    _colors = _coerceColors(effective['colors'], _color);
    _width = coerceDouble(effective['width'] ?? effective['border_width']) ?? 1.4;
    _radius = coerceDouble(effective['radius'] ?? effective['border_radius']) ?? 0;
    _side = (effective['side'] ?? 'all').toString().toLowerCase();
    _variant = (effective['variant'] ?? effective['style'] ?? 'solid')
        .toString()
        .toLowerCase();
    _durationMs =
        (coerceOptionalInt(effective['duration_ms'] ?? effective['duration']) ??
                1600)
            .clamp(1, 600000);
    _glow = effective['glow'] == true || effective['variant'] == 'glow';
    _glowColor = coerceColor(effective['glow_color']) ?? _colors.first;
    _dashLength = coerceDouble(effective['dash_length']) ?? 10.0;
    _dashGap = coerceDouble(effective['dash_gap']) ?? 6.0;
    _animated =
        effective['animated'] != false &&
        _variant != 'solid' &&
        _variant != 'static';
  ...
  Widget build(BuildContext context) {
    ...
    final padding =
        coercePadding(effective['padding']) ?? const EdgeInsets.all(8);
  ...
  Map<String, Object?> _effectiveProps(Map<String, Object?> props) {
    return resolveStylingHelperProps(props, controlType: 'border');
  }
}
