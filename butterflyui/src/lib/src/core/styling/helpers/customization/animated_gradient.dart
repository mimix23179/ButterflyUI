...
class _AnimatedGradientControlState extends State<_AnimatedGradientControl>
  ...
  void _syncFromProps(
    ...
    final effective = _effectiveProps(props);
    _colors = _coerceColors(effective['colors']);
    _stops = _coerceStops(effective['stops'], _colors.length);
    _durationMs =
        coerceOptionalInt(effective['duration_ms'] ?? effective['duration']) ??
        1800;
    _radius = coerceDouble(effective['radius']) ?? 0;
    _angle = coerceDouble(effective['angle'] ?? effective['start_angle']) ?? 0;
    _loop = effective['loop'] != false;
    _playing =
        effective['playing'] == true ||
        effective['play'] == true ||
        effective['autoplay'] != false;
    _pingPong = effective['ping_pong'] == true;
    _shift = effective['shift'] == true;
  ...
  Widget build(BuildContext context) {
    ...
    final begin = _alignmentFrom(effective['begin']) ?? Alignment.topLeft;
    final end = _alignmentFrom(effective['end']) ?? Alignment.bottomRight;
    final center = _alignmentFrom(effective['center']) ?? Alignment.center;
    final variant =
        (effective['variant'] ??
                effective['kind'] ??
                effective['type'])
      ...
      builder: (context, renderedChild) {
        ...
        final gradient = switch (variant) {
          'radial' || 'radial_gradient' => RadialGradient(
            ...
            stops: stops,
            center: center,
            radius: coerceDouble(effective['radius']) ?? 0.8,
            tileMode: _tileMode(effective['tile_mode']),
          ),
          'sweep' || 'conic' || 'sweep_gradient' => SweepGradient(
            ...
            center: center,
            startAngle:
                (coerceDouble(effective['start_angle']) ?? 0) *
                math.pi /
                180,
            endAngle:
                (coerceDouble(effective['end_angle']) ?? 360) *
                math.pi /
                180,
  ...
  Map<String, Object?> _effectiveProps(Map<String, Object?> props) {
    return resolveStylingHelperProps(props, controlType: 'animated_gradient');
  }
}
