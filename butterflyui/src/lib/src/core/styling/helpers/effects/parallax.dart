  ...
  return buildRuntimePropsControl(
    ...
    builder: (liveProps) {
      final resolvedProps = resolveStylingHelperProps(
        liveProps,
        controlType: 'parallax',
      );
      Widget child = const SizedBox.shrink();
      if (rawChildren.isNotEmpty && rawChildren.first is Map) {
        child = buildChild(coerceObjectMap(rawChildren.first as Map));
      } else if (resolvedProps['child'] is Map) {
        child = buildChild(coerceObjectMap(resolvedProps['child'] as Map));
      }

      return wrapWithEffectRenderLayers(
        controlId: controlId.isEmpty ? 'parallax' : controlId,
        props: resolvedProps,
        child: _ParallaxControl(props: resolvedProps, child: child),
      );
    },
  );
}
