...
class _FlowFieldControlState extends State<_FlowFieldControl>
  ...
  void initState() {
    super.initState();
    _seed = coerceOptionalInt(_effectiveProps(widget.props)['seed']) ?? 1;
  ...
  void didUpdateWidget(covariant _FlowFieldControl oldWidget) {
    ...
    if (!mapEquals(oldWidget.props, widget.props)) {
      _seed = coerceOptionalInt(_effectiveProps(widget.props)['seed']) ?? _seed;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final density =
        (coerceDouble(effective['density']) ?? 24).clamp(8.0, 120.0);
    final speed =
        (coerceDouble(effective['speed']) ?? 0.35).clamp(0.0, 5.0);
    final lineWidth =
        (coerceDouble(effective['line_width']) ?? 1.1).clamp(0.2, 8.0);
    final opacity =
        (coerceDouble(effective['opacity']) ?? 0.6).clamp(0.0, 1.0);
    final color =
        (coerceColor(effective['color']) ?? const Color(0xff22d3ee))
    ...
    return wrapWithEffectRenderLayers(
      controlId: widget.controlId.isEmpty ? 'flow_field' : widget.controlId,
      props: effective,
  ...
  Map<String, Object?> _effectiveProps(Map<String, Object?> props) {
    return resolveStylingHelperProps(props, controlType: 'flow_field');
  }
}
