  ...
  return buildRuntimePropsControl(
    ...
    builder: (liveProps) {
      final resolvedProps = resolveStylingHelperProps(
        liveProps,
        controlType: 'animated_background',
      );
      final colors = _coerceColorList(resolvedProps['colors']);
      final hasEffectScene =
          !resolveEffectSceneFromProps(resolvedProps).isEmpty;
      ...
      final durationMs =
          coerceOptionalInt(
            resolvedProps['duration_ms'] ?? resolvedProps['duration'],
          ) ??
          2400;
      final loop = resolvedProps['loop'] == null
          ? true
          : (resolvedProps['loop'] == true);
      final curve = _parseCurve(resolvedProps['curve']);
