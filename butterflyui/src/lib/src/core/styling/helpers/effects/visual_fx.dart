...
Widget buildVisualFxControl(
  ...
  final resolved = resolveStylingHelperProps(props, controlType: 'visual_fx');
  final glowProps = _mergeVisualFxProps(resolved, 'glow');
  final glassProps = _mergeVisualFxProps(resolved, 'glass_blur');
  final chromaticProps = _mergeVisualFxProps(resolved, 'chromatic_shift');
  final sweepProps = _mergeVisualFxProps(resolved, 'gradient_sweep');

  final enableGlow = _coerceBool(
    glowProps['enabled'] ?? resolved['enable_glow'],
    fallback: true,
  );
  final enableGlass = _coerceBool(
    glassProps['enabled'] ?? resolved['enable_glass_blur'],
    fallback: true,
  );
  final enableChromatic = _coerceBool(
    chromaticProps['enabled'] ?? resolved['enable_chromatic_shift'],
    fallback: true,
  );
  final enableSweep = _coerceBool(
    sweepProps['enabled'] ?? resolved['enable_gradient_sweep'],
    fallback: true,
  );
  ...
  return wrapWithEffectRenderLayers(
    controlId: controlId.isEmpty ? 'visual_fx::layers' : '$controlId::layers',
    props: resolved,
    child: current,
  );
}
    ...
    switch (method) {
      ...
      case 'set_colors':
        ...
        widget.sendEvent(widget.controlId, 'change', {
          'colors': _colors.map((c) => c.toARGB32()).toList(growable: false),
        });
        return true;
      default:
        ...
        child: Container(
          decoration: BoxDecoration(
            ...
            boxShadow: borderGlow == null
                ...
                : [
                    BoxShadow(
                      color: borderGlow.withValues(
                        alpha: (borderGlow.a * 0.35).clamp(0.0, 1.0),
                      ),
                      blurRadius: 16,
                      spreadRadius: 1,
                    ),
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final paint = Paint()
      ..color = Colors.white.withValues(
        alpha: (Colors.white.a * opacity).clamp(0.0, 1.0),
      );
    for (var i = 0; i < 350; i++) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      final paint = Paint()
        ..color = colors[i % colors.length].withValues(
          alpha: (colors[i % colors.length].a * alpha).clamp(0.0, 1.0),
        );
      canvas.drawRect(
