...
Widget buildEffectsControl(
  ...
  return buildRuntimePropsControl(
    ...
    builder: (liveProps) {
      ...
      if (resolvedProps['enabled'] == false) return child;

      Widget built = child;

      final blur = coerceDouble(resolvedProps['blur']) ?? 0.0;
      if (blur > 0) {
        built = ImageFiltered(
      ...
      final color = coerceColor(resolvedProps['color']);
      final blendMode =
          _parseBlendMode(resolvedProps['blend_mode']) ?? BlendMode.srcATop;
      if (color != null) {
        built = ColorFiltered(
      ...
      final matrix = _buildColorMatrix(resolvedProps);
      if (matrix != null) {
        built = ColorFiltered(
      ...
      final opacity = coerceDouble(resolvedProps['opacity']);
