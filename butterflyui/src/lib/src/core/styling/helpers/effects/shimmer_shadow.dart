...
Widget buildShimmerShadowControl(
  ...
  final resolved = resolveStylingHelperProps(
    props,
    controlType: 'shimmer_shadow',
  );
  final shimmerProps = _mergePropsSection(resolved, 'shimmer');
  final shadowProps = _mergePropsSection(resolved, 'shadow');
  final withShadow = buildShadowStackControl(shadowProps, child);
  return wrapWithEffectRenderLayers(
    controlId: controlId.isEmpty
        ? 'shimmer_shadow::layers'
        : '$controlId::layers',
    props: resolved,
  ...
  Widget build(BuildContext context) {
    final baseColor =
        ...
        Theme.of(context).colorScheme.surfaceContainerHighest.withValues(
          alpha: (
            Theme.of(context).colorScheme.surfaceContainerHighest.a * 0.45
          ).clamp(0.0, 1.0),
        );
    final highlightColor =
        coerceColor(widget.props['highlight_color']) ??
        Theme.of(context).colorScheme.surface.withValues(
          alpha: (Theme.of(context).colorScheme.surface.a * 0.75).clamp(
            0.0,
            1.0,
          ),
        );
