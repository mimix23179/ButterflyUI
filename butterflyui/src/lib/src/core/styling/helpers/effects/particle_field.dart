...
class _ParticleFieldControlState extends State<_ParticleFieldControl> {
  ...
  void initState() {
    ...
    final playValue = effective.containsKey('play')
        ? effective['play']
        : (effective.containsKey('playing')
              ? effective['playing']
              : effective['autoplay']);
    _play = playValue == null ? true : (playValue == true);
    _seed = coerceOptionalInt(effective['seed']);
  ...
  Widget build(BuildContext context) {
    final effective = _effectiveProps(widget.props);
    final count = (coerceOptionalInt(effective['count']) ?? 40)
        .clamp(0, 2000)
        .toInt();
    final colors = _coerceColorList(effective['colors']);
    final size = coerceDouble(effective['size']);
    final minSize = coerceDouble(effective['min_size']) ?? size ?? 2.0;
    final maxSize = coerceDouble(effective['max_size']) ?? size ?? 6.0;
    final speed = coerceDouble(effective['speed']);
    final minSpeed = coerceDouble(effective['min_speed']) ?? speed ?? 8.0;
    final maxSpeed = coerceDouble(effective['max_speed']) ?? speed ?? 32.0;
    final direction = coerceDouble(
      effective['direction'] ??
          effective['direction_deg'] ??
          effective['direction_degrees'],
    );
    final spread = coerceDouble(effective['spread']);
    final opacity =
        (coerceDouble(effective['opacity']) ?? 0.6).clamp(0.0, 1.0);
    final loop = effective['loop'] == null
        ? true
        : (effective['loop'] == true);
    final shape = effective['shape']?.toString() ?? 'circle';

    return wrapWithEffectRenderLayers(
      controlId: widget.controlId.isEmpty ? 'particle_field' : widget.controlId,
      props: effective,
      child: ButterflyUIParticleField(
  ...
  Map<String, Object?> _effectiveProps(Map<String, Object?> props) {
    return resolveStylingHelperProps(props, controlType: 'particle_field');
  }
}
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final particle in particles) {
      ...
      final color = particle.color.withValues(
        alpha: (particle.color.a * opacity).clamp(0.0, 1.0),
      );
      paint.color = color;
      if (drawSquare) {
