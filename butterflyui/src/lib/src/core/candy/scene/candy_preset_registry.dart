...
void _addCandyPresetLayers(
  List<CandySceneLayer> layers,
  Set<String> signatures,
  String token,
) {
  void add(Object? value, {CandySceneLayerPosition? position}) {
    final layer = CandySceneLayer.fromValue(value, defaultPosition: position);
  ...
  switch (normalizeCandySceneLayerToken(token)) {
    case 'galaxy':
      add(<String, Object?>{
        'type': 'nebula',
        'density': 0.82,
    ...
    case 'rainstorm':
      add(<String, Object?>{
        ...
        'accent_color': '#D7F2FF',
        'opacity': 0.55,
      }, position: CandySceneLayerPosition.overlay);
      break;
    case 'water':
    case 'water_flow':
    case 'flowing_water':
...
CandyScene resolveCandySceneFromProps(Map<String, Object?> props) {
  final layers = <CandySceneLayer>[];
  final signatures = <String>{};

  void addValue(Object? value, {CandySceneLayerPosition? position}) {
    ...
    if (value is Map) {
      ...
      if (nestedSceneLayers != null ||
        ...
        addValue(
          ...
          position: CandySceneLayerPosition.background,
        );
        addValue(
          nestedOverlayLayers,
          position: CandySceneLayerPosition.overlay,
  ...
  addValue(props['overlay_layers'], position: CandySceneLayerPosition.overlay);
  ...
  if (!hasExplicitSceneLayers || mergePresetScene) {
    final semanticTokens = <String>[
      ...
      ..._coerceCandyStringList(props['ambient']),
      ..._coerceCandyStringList(props['reactive']),
      ..._coerceCandyStringList(props['decor']),
    ];
    for (final token in semanticTokens) {
      _addCandyPresetLayers(layers, signatures, token);
    }
  }

  return CandyScene.fromLayers(layers);
}
