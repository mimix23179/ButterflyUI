...
class Particles(CandySubmodule):
    """Candy submodule host control for `particles`."""

    control_type = "candy_particles"
    umbrella = "candy"
    ...
    def __init__(
        ...
        **kwargs: object,
        ...
        if drift is not None:
            resolved_payload["drift"] = drift
        if gravity is not None:
            resolved_payload["gravity"] = gravity
        if overlay is not None:
            resolved_payload["overlay"] = overlay
        if size is not None:
            resolved_payload["size"] = size
        if speed is not None:
            resolved_payload["speed"] = speed
        super().__init__(
            *children,
