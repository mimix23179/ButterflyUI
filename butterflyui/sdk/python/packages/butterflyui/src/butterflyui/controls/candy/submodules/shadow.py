...
class Shadow(CandySubmodule):
    """Candy submodule host control for `shadow`."""

    control_type = "candy_shadow"
    umbrella = "candy"
    ...
    def __init__(
        ...
        **kwargs: object,
        ...
        if dx is not None:
            resolved_payload["dx"] = dx
        if dy is not None:
            resolved_payload["dy"] = dy
        if spread is not None:
            resolved_payload["spread"] = spread
        super().__init__(
            *children,
