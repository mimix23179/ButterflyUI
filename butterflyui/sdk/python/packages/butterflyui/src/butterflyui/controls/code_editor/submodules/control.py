...
from ..control import CODE_EDITOR_EVENTS, CODE_EDITOR_SCHEMA_VERSION, CODE_EDITOR_STATES, CodeEditor
from .schema import (
...
def _sanitize_module_payload(module: str, payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    allowed_keys = set(CODE_EDITOR_COMMON_PAYLOAD_KEYS) | set(MODULE_ALLOWED_KEYS.get(normalized_module, set()))
    type_map = dict(CODE_EDITOR_COMMON_PAYLOAD_TYPES)
    type_map.update(MODULE_PAYLOAD_TYPES.get(normalized_module, {}))
    allowed_events = {str(event) for event in CODE_EDITOR_EVENTS}
    allowed_states = {str(state) for state in CODE_EDITOR_STATES}
...
class CodeEditorSubmodule(CodeEditor):
    ...
    def __init_subclass__(cls, **kwargs: Any) -> None:
        ...
        if module_token:
            ...
            cls.supported_events = tuple(MODULE_EVENTS.get(module_token, tuple(sorted(CODE_EDITOR_EVENTS))))
        ...
        all_props.extend(CODE_EDITOR_COMMON_PAYLOAD_KEYS)
        if module_token:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if not module_token:
            raise ValueError("CodeEditorSubmodule requires module_token on subclass")
        if not canonical_module:
            canonical_module = module_token
        ...
        if normalized_events is None:
            normalized_events = [event for event in self.supported_events if event in CODE_EDITOR_EVENTS]
            if not normalized_events:
                normalized_events = sorted(CODE_EDITOR_EVENTS)

        resolved_value = code if code is not None else text if text is not None else value
        merged = merge_props(
            props,
    ...
    def run_action(
        ...
        **kwargs: Any,
        ...
        if normalized_action not in set(self.supported_actions):
            return {
                "ok": False,
                "error": f"Unsupported code_editor submodule action '{normalized_action}' for module '{self.canonical_module}'",
            }
    ...
    def emit_event(
        ...
        **kwargs: Any,
        ...
        if normalized_event not in set(self.supported_events):
            return {
                "ok": False,
                "error": f"Unsupported code_editor submodule event '{normalized_event}' for module '{self.canonical_module}'",
            }
        payload_dict = _coerce_mapping(payload)
        if kwargs:
            payload_dict.update(dict(kwargs))
