...
from ..control import STUDIO_SCHEMA_VERSION, STUDIO_EVENTS, STUDIO_STATES, _normalize_studio_module, Studio
from .schema import (
...
def _sanitize_module_payload(module: str, payload: Mapping[str, Any]) -> dict[str, Any]:
    normalized_module = _normalize_studio_module(module) or _normalize_token(module)
    allowed_keys = set(STUDIO_COMMON_PAYLOAD_KEYS) | set(MODULE_ALLOWED_KEYS.get(normalized_module, set()))
    type_map = dict(STUDIO_COMMON_PAYLOAD_TYPES)
    type_map.update(MODULE_PAYLOAD_TYPES.get(normalized_module, {}))

    allowed_events = {str(event) for event in STUDIO_EVENTS}
    allowed_states = {str(state) for state in STUDIO_STATES}
...
class StudioSubmodule(Studio):
    ...
    base_props: tuple[str, ...] = ('schema_version', 'module', 'module_id', 'state', 'custom_layout', 'layout', 'show_modules', 'show_chrome', 'radius', 'events', 'manifest', 'registries', 'modules', 'panels', 'shortcuts', 'render', 'cache', 'media', 'documents', 'assets', 'left_pane_ratio', 'right_pane_ratio', 'bottom_panel_height')
    supported_props: tuple[str, ...] = base_props

    def __init_subclass__(cls, **kwargs: Any) -> None:
        super().__init_subclass__(**kwargs)
        module_token = _normalize_studio_module(str(getattr(cls, "module_token", ""))) or _normalize_token(
            str(getattr(cls, "module_token", ""))
        )
        canonical_module = _normalize_studio_module(str(getattr(cls, "canonical_module", module_token))) or module_token
        ...
        if module_token:
            ...
            cls.supported_events = tuple(MODULE_EVENTS.get(module_token, tuple(sorted(STUDIO_EVENTS))))
        ...
        all_props.extend(STUDIO_COMMON_PAYLOAD_KEYS)
        if module_token:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        module_token = _normalize_studio_module(str(getattr(self, "module_token", ""))) or _normalize_token(
            str(getattr(self, "module_token", ""))
        )
        canonical_module = _normalize_studio_module(str(getattr(self, "canonical_module", module_token))) or module_token
        if not module_token:
            raise ValueError("StudioSubmodule requires module_token on subclass")
        if not canonical_module:
            canonical_module = module_token
    ...
    def set_payload(self, session: Any, payload: Mapping[str, Any] | None = None, **kwargs: Any) -> dict[str, Any]:
        ...
        module_name = _normalize_studio_module(str(getattr(self, "canonical_module", self.module_token))) or _normalize_token(
        ...
        **kwargs: Any,
        ...
        if normalized_action not in set(self.supported_actions):
            return {
                "ok": False,
                "error": (
                    "Unsupported studio submodule action "
        ...
        **kwargs: Any,
        ...
        if normalized_event not in set(self.supported_events):
            return {
                "ok": False,
                "error": (
                    "Unsupported studio submodule event "
