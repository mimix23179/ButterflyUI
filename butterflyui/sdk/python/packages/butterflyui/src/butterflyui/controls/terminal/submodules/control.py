...
from ..control import TERMINAL_SCHEMA_VERSION, TERMINAL_EVENTS, TERMINAL_STATES, _normalize_module, Terminal
from .schema import (
...
def _normalize_dependencies(values: Any) -> tuple[str, ...]:
    ...
    if isinstance(values, (list, tuple, set)):
        for item in values:
            token = _normalize_module(str(item)) or _normalize_token(str(item))
            if token and token not in out:
                out.append(token)
    elif values is not None:
        token = _normalize_module(str(values)) or _normalize_token(str(values))
        if token:
            out.append(token)
    return tuple(out)


def _module_metadata(submodule: "TerminalSubmodule") -> dict[str, Any]:
    canonical_module = _normalize_module(
    ...
    module_token = _normalize_module(str(getattr(submodule, "module_token", canonical_module))) or canonical_module
...
def _sanitize_module_payload(module: str, payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    allowed_keys = set(TERMINAL_COMMON_PAYLOAD_KEYS) | set(MODULE_ALLOWED_KEYS.get(normalized_module, set()))
    type_map = dict(TERMINAL_COMMON_PAYLOAD_TYPES)
    type_map.update(MODULE_PAYLOAD_TYPES.get(normalized_module, {}))

    allowed_events = {str(event) for event in TERMINAL_EVENTS}
    allowed_states = {str(state) for state in TERMINAL_STATES}
...
class TerminalSubmodule(Terminal):
    module_token: str = ""
    ...
    supported_events: tuple[str, ...] = tuple(sorted(TERMINAL_EVENTS))
    ...
    base_props: tuple[str, ...] = ('schema_version', 'module', 'module_id', 'state', 'custom_layout', 'layout', 'events', 'manifest', 'registries', 'modules', 'lines', 'output', 'raw_text', 'show_input', 'read_only', 'clear_on_submit', 'strip_ansi', 'auto_scroll', 'wrap_lines', 'max_lines', 'border_width', 'engine', 'webview_engine')
    ...
    def __init_subclass__(cls, **kwargs: Any) -> None:
        ...
        if module_token:
            ...
            cls.supported_events = tuple(MODULE_EVENTS.get(module_token, tuple(sorted(TERMINAL_EVENTS))))
        ...
        all_props.extend(TERMINAL_COMMON_PAYLOAD_KEYS)
        if module_token:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if not module_token:
            raise ValueError("TerminalSubmodule requires module_token on subclass")
        if not canonical_module:
            canonical_module = module_token
        ...
        **kwargs: Any,
        ...
        if normalized_action not in set(self.supported_actions):
            return {
                "ok": False,
                "error": (
                    "Unsupported terminal submodule action "
        ...
        **kwargs: Any,
        ...
        if normalized_event not in set(self.supported_events):
            return {
                "ok": False,
                "error": (
                    "Unsupported terminal submodule event "
    ...
    def describe_contract(self) -> dict[str, Any]:
        ...
        return {
            ...
            "control_type": getattr(self, "control_type", "terminal"),
            "id": metadata["id"],
