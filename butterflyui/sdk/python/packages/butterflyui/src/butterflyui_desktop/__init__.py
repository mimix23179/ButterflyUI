...
RUNTIME_ZIP_URL: str = "https://www.dropbox.com/scl/fi/umfxqx5jmx7610uzi47s3/butterflyui-windows.zip?rlkey=7pdctzo963exxs2nt7x93sjx0&st=0qvc0p7a&dl=0"  # <-- set your URL
...
class ButterflyUIDesktopInstallError(RuntimeError):
    pass


class ButterflyUIDesktopRunError(RuntimeError):
    pass
...
def _find_installed_package_dir() -> Optional[Path]:
    """Try to locate the installed butterflyui_desktop package directory.
    ...
    for root in candidates:
        pkg_dir = root / "butterflyui_desktop"
...
def get_runtime_dir(*, use_env_runtime_dir: bool = True) -> Path:
    ...
    Resolution order:
    1. `BUTTERFLYUI_DESKTOP_RUNTIME_DIR` env var if set (when use_env_runtime_dir=True).
    ...
    if use_env_runtime_dir:
        env = os.environ.get("BUTTERFLYUI_DESKTOP_RUNTIME_DIR", "").strip()
        if env:
            return Path(env)
...
def _require_https(url: str) -> None:
    ...
    if parsed.scheme.lower() != "https":
        raise ButterflyUIDesktopInstallError("Runtime URL must use https://")
...
def _download_to_temp(url: str) -> Path:
    ...
    _require_https(url)
    tmp = Path(tempfile.mkdtemp(prefix="butterflyui_desktop_"))
    dest = tmp / "butterflyui-windows.zip"

    attempt = 0
    tried_dropbox_dl = False
    while True:
        attempt += 1
        req = urllib.request.Request(url, headers={"User-Agent": "butterflyui-desktop/0"})
        ...
        raise ButterflyUIDesktopInstallError(
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        for member in zf.infolist():
            ...
            if p.is_absolute() or any(part == ".." for part in p.parts):
                raise ButterflyUIDesktopInstallError(f"Unsafe path in zip: {name}")
...
def get_installed_info(runtime_dir: Optional[Path] = None, *, use_env_runtime_dir: bool = True) -> Optional[dict]:
    ...
    test for the presence of a `ButterflyUI.exe` (case-insensitive) under the
    runtime directory.
    """
...
def install(
    *,
    ...
    """Install the butterflyui-windows.zip into the package `app/` folder.
    ...
    env_url = os.environ.get("BUTTERFLYUI_DESKTOP_RUNTIME_URL", "").strip()
    env_sha256 = os.environ.get("BUTTERFLYUI_DESKTOP_RUNTIME_SHA256", "").strip()
    url = (url or env_url or RUNTIME_ZIP_URL or "").strip()
    sha256 = (sha256 or env_sha256 or RUNTIME_ZIP_SHA256 or "").strip()

    if not url:
        raise ButterflyUIDesktopInstallError(
            "RUNTIME_ZIP_URL is not set. Edit butterflyui_desktop.__init__.py and set RUNTIME_ZIP_URL to your runtime zip HTTPS URL."
        )
    ...
    zip_path = _download_to_path(url, runtime_dir / "butterflyui-windows.zip")
...
def find_butterflyui_exe(runtime_dir: Optional[Path] = None, *, use_env_runtime_dir: bool = True) -> Path:
    runtime_dir = runtime_dir or get_runtime_dir(use_env_runtime_dir=use_env_runtime_dir)
    if not runtime_dir.exists():
        raise ButterflyUIDesktopRunError("Runtime not installed; call install() first")
...
def run(
    *,
    ...
    """Run ButterflyUI.exe with optional host:port (no URL schemes).

    Returns subprocess.Popen when wait=False, otherwise returns the exit code.
    """

    if not sys.platform.startswith("win"):
        raise ButterflyUIDesktopRunError("ButterflyUI runtime currently supported on Windows only")

    runtime_dir = runtime_dir or get_runtime_dir(use_env_runtime_dir=use_env_runtime_dir)
    exe = find_butterflyui_exe(runtime_dir)
    args = [str(exe)]
    if host_port:
    ...
    if host_port:
        ...
        env.setdefault("BUTTERFLYUI_WS_URL", host_port)
    if session_token:
        env["BUTTERFLYUI_SESSION_TOKEN"] = session_token
...
__all__ = [
    "install",
    "find_butterflyui_exe",
    "run",
    "get_runtime_dir",
    "get_installed_info",
    "ButterflyUIDesktopInstallError",
    "ButterflyUIDesktopRunError",
]
